...
В лаунчере используется одно основное меню:
- `1` и `2` — выбор режима работы (`last24h` / `watch`),
- `0` — переход в меню выбора модели.

Выбранная модель сохраняется в `launcher_settings.json` и автоматически используется при следующем запуске.
По умолчанию — `llama`, пока вы явно не выберете другую модель.
